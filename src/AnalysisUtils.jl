...
function surveydf(df::DataFrame, symb::Tuple = ())
    ...
    show(size(df))
    println("\n")
    show(sort(describe(df, :min, :max, :nmissing, :nuniqueall, :eltype), :variable), allcols = true, allrows = true, truncate = 0)
    println("\n")
    header("Columns in order of appearance in the data frame", sep = ".")
    show(map(x -> (name = x, ) , names(pumpdf)) |> DataFrame; allcols = true, allrows = true, truncate = 0)
    println("\n")
    # show(skim(df))
    println("\n")
    if length(symb) > 0
